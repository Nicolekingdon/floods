...
flood1 <- flood1 |>
  distinct(totalNumberIaApproved, totalAmountIhpApproved, totalAmountHaApproved, totalAmountOnaApproved)

flood1 |>
  kbl(caption = "SEVERE, STORMS, FLOODING, LANDSLIDES, AND MUDSLIDES") |>
  kable_minimal()
...
flood2 |>
  kbl(caption = "SEVERE STORMS, TORNADOES, AND FLOODING") |>
  kable_minimal()
```
...
flood3 |>
  kbl(caption = "SEVERE STORM AND FLOODING") |>
  kable_minimal()
```
...
flood7 |>
  kbl(caption = "SEVERE STORMS, FLOODING, LANDSLIDES, AND MUDSLIDE") |>
  kable_minimal()
```
