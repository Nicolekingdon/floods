...
subtitle: "An Exploratory Data Analysis of Assistance by Length of Flood, Severity of Flood, and Total Population Living in Poverty and with Children and Adolescents"
...
As risks factors stemming from disasters are inevitable, it is important to delineate how post-flood recovery programs differed for counties with the lowest income populations. This Exploratory Data Analysis will focus on examining what counties were declared to receive the IHA, PA, and/or HM program, which counties had the highest total populations living in poverty, as well as the most children 0 to 17 years of age. It will also examine the different lengths of the floods, as it would be perceived that longer floods would amount to more damage. Finally, this study aims to use a strengths-based lens, by offering suggestions on how protective factors, like FEMA assistance programs, could be bolstered for economically marginalized populations.
...
Finally, I finished cleaning the `census_2023` data frame by cleaning up the column names.
...
Next, I prepped the data frame to be able to be only holding data from 2020 and 2021. To do this, I split the `incidentBeginDate` column to reveal a single column of `incident_begin_year` to support in this step.
...
To begin, I trimmed the strings in both data frames so that the names could be simply both of the counties only, as some included the word county and the state. I also renamed both columns to `county`.
...
I also used Lubridate to make both the `incident_begin_date` and `incident_end_date` columns into dates. As well as, mutated a new column, `incident_length_days`, with the difference in days between the incident begin and end date.
...
The following values in the `county` column are nominal variables: `r paste(storm_20_21|> distinct(county))`.

### `declarationTitle`

The `declarationTitle` column holds the following distinct values: `r paste(storm_20_21|> distinct(declarationTitle))`. Each values describes what type of flood occured in the particular county for the particular date. These values are all strings, as well as are nominal variables.
...
The three `Estimate_Total_Population` columns, `Estimate_Total_Population_for_whom_poverty_status_is_determined`,
...
## Flood Type Declaration

I prepped data frames by flood type to see the amount of grant funding provided to each county.
...
The counties that experienced `SEVERE STORM AND FLOODING` received the following amounts for Individual Assistance: \$955, Individuals and Housing Assistance: \$8,138,894, Housing Assistance: \$6,301,770, and Other Needs Assistance: \$1,837,123. All counties received the same amount for all types of assistance.

### `SEVERE STORMS, FLOODING, LANDSLIDES, AND MUDSLIDE`

Counties with `SEVERE STORMS, FLOODING, LANDSLIDES, AND MUDSLIDE` received the following amounts for Individual Assistance: \$165, Individuals and Housing Assistance: \$2,242,253, Housing Assistance: \$2,069,845, and Other Needs Assistance: \$172,408.40. Each county received the same amount.

### Individual Assistance Program

Figure 2 shows the counties with the highest populations living in poverty, as well as how long their floods were and if they were offered the FEMA IA program. Figure 3 shows such for total population living in poverty under five years of age, and Figure 4 shows the same for children 5-to-17 year olds. All figures show that for floods occurring in 2020 and 2021, these counties did not receive the IA program, regardless of their total population living in poverty and length of flood.
...
ggplot(data = storm_20_21,
  ...
  facet_wrap(~iaProgramDeclared, nrow = 1) +
  labs(
    title = "Figure 3. Individuals Assistance Program Declaration",
...
ggplot(data = storm_20_21,
  ...
  facet_wrap(~iaProgramDeclared, nrow = 1) +
  labs(
    title = "Figure 4. Individual Assistance Program Declaration",
...
ggplot(data = storm_20_21,
  ...
  labs(
    title = "Figure 5. Individual and Household Assistance Program Declaration",
    subtitle = "Total Population in Poverty by County",
    x = "Length of Flood in Days",
    y = "Population living in Poverty (10000s)"
  ) +
...
ggplot(data = storm_20_21,
  ...
  facet_wrap(~ihProgramDeclared, nrow = 1) +
  labs(
    title = "Figure 6. Individual and Household Assistance Program Declaration",
    subtitle = "Total Population in Poverty Under the Age of 5 by County",
    x = "Length of Flood in Days",
    y = "Population Under 5 Years in Poverty (10000s)"
  ) +
...
ggplot(data = storm_20_21,
  ...
  facet_wrap(~ihProgramDeclared, nrow = 1) +
  labs(
    title = "Figure 7. Individual and Household Assistance Program Declaration",
    subtitle = "Total Population in Poverty from 5-to-17 Years of Age by County",
    x = "Length in Days",
    y = "Population 5-to-17 Years in Poverty (10000s)"
  ) +
...
ggplot(data = storm_20_21,
  ...
  labs(
    title = "Figure 8. Public Assistance Program Declaration",
    subtitle = "Total Population in Poverty by County",
    x = "Length of Flood in Days",
    y = "Population living in Poverty (10000s)"
  ) +
...
ggplot(data = storm_20_21,
  ...
  facet_wrap(~paProgramDeclared, nrow = 1) +
  labs(
    title = "Figure 9. Public Assistance Program Declaration",
    subtitle = "Total Population in Poverty Under the Age of 5 by County",
    x = "Length of Flood in Days",
    y = "Population Under 5 Years in Poverty (10000s)"
  ) +
...
ggplot(data = storm_20_21,
  ...
  facet_wrap(~paProgramDeclared, nrow = 1) +
  labs(
    title = "Figure 10. Public Assistance Program Declaration",
    subtitle = "Total Population in Poverty from 5-to-17 Years of Age by County",
    x = "Length in Days",
    y = "Population 5-to-17 Years in Poverty (10000s)"
  ) +
...
ggplot(data = storm_20_21,
  ...
  labs(
    title = "Figure 11. Hazard Mitigation Program Declaration",
    subtitle = "Total Population in Poverty by County",
    x = "Length of Flood in Days",
    y = "Population living in Poverty (10000s)"
  ) +
